...
if ($action === 'reset' && confirm_sesskey()) {
    $DB->execute("DELETE g FROM {local_xpstore_gastos} g JOIN {course_modules} cm ON g.itemid = cm.id WHERE cm.course = ?", array($courseid));
    redirect($url, get_string('productdeleted', 'local_xpstore'));
}

// Pre-cargar categorías de la tienda para asignarlas a los registros (CORREGIDO: Lee el catálogo del curso)
...
<style>
    ...
    .report-wrapper { font-family: 'Montserrat', sans-serif; background: #f4f6f9; padding: 30px; border-radius: 20px; }
    ...
    .header-title p { color: #777; margin: 0; font-size: 0.95rem; }
    
    .btn-tienda { background: white; border: 1px solid #ccc; color: #555; padding: 8px 20px; border-radius: 50px; font-weight: bold; text-decoration: none !important; margin-right: 10px; transition: 0.2s; }
    .btn-tienda:hover { background: #f0f0f0; }
    .btn-reset { background: #c93a3a; color: white; padding: 8px 20px; border-radius: 50px; font-weight: bold; text-decoration: none !important; border: none; transition: 0.2s; }
    .btn-reset:hover { background: #a82e2e; color: white; }
    
    .user-card { background: white; border-radius: 15px; margin-bottom: 15px; box-shadow: 0 4px 6px rgba(0,0,0,0.02); overflow: hidden; }
    .card-header-user { display: flex; align-items: center; padding: 15px 25px; cursor: pointer; transition: 0.2s; }
    .card-header-user:hover { background: #fafafa; }
    
    .u-avatar-container img.userpicture { width: 50px !important; height: 50px !important; border-radius: 50%; border: 2px solid #7d2ae8; margin-right: 15px; display: block; }
    
    .u-info { flex-grow: 1; min-width: 0; padding-right: 10px; } 
    .u-name { font-weight: bold; font-size: 1.1rem; color: #222; margin-bottom: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .u-email { color: #888; font-size: 0.85rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    
    .badge-total { background: #7d2ae8; color: white; padding: 5px 15px; border-radius: 20px; font-weight: bold; font-size: 0.85rem; margin-right: 15px; white-space: nowrap; flex-shrink: 0; }
    
    .card-body-logs { display: none; padding: 20px 25px; border-top: 1px solid #eee; background: #fff; }
    .table-logs { width: 100%; border-collapse: collapse; }
    .table-logs th { text-align: left; padding: 10px; color: #aaa; font-size: 0.75rem; text-transform: uppercase; letter-spacing: 1px; border-bottom: 2px solid #f0f0f0; }
    .table-logs td { padding: 15px 10px; border-bottom: 1px solid #f5f5f5; vertical-align: middle; }
    
    .badge-tipo { padding: 5px 12px; border-radius: 8px; font-size: 0.75rem; font-weight: bold; color: white; white-space: nowrap; }
    ...
    @media (max-width: 768px) {
        ...
        .u-avatar-container img.userpicture { width: 40px !important; height: 40px !important; margin-right: 10px; }
        .u-name { font-size: 1rem; }
        .u-email { font-size: 0.75rem; }
        .badge-total { padding: 5px 10px; font-size: 0.75rem; margin-right: 10px; }
        .card-body-logs { padding: 15px; overflow-x: auto; -webkit-overflow-scrolling: touch; }
        .table-logs th, .table-logs td { white-space: nowrap; padding: 10px 8px; }
    }
</style>
...
<div class="report-wrapper">
    <div class="header-container">
        ...
        <div>
            ...
            <a href="<?php echo $url->out(false); ?>&action=reset&sesskey=<?php echo sesskey(); ?>" class="btn-reset" onclick="return confirm('<?php echo get_string('confirmreset', 'local_xpstore'); ?>')">
    ...
    ?>
        <div class="user-card">
            <div class="card-header-user" onclick="toggleLogs(<?php echo $userid; ?>)">
                ...
                <div class="u-info">
                    <div class="u-name">
                        <a href="<?php echo $profile_url; ?>" onclick="event.stopPropagation();" style="color: inherit; text-decoration: none;" onmouseover="this.style.textDecoration='underline'" onmouseout="this.style.textDecoration='none'" target="_blank">
                            <?php echo $user_fullname; ?>
                        </a>
                    </div>
                    <div class="u-email"><?php echo $real_user->email; ?></div>
                </div>
                <div class="badge-total"><?php echo $total_canjes . ' ' . get_string('redemptions', 'local_xpstore'); ?></div>
                    ...
                    <tbody>
                        ...
                        foreach ($user_logs as $log) {
                            ...
                            if ($tipo_str === 'g') {
                                ...
                                $cm_url = new moodle_url('/grade/report/user/index.php', array('id' => $courseid, 'userid' => $log->userid));
                            ...
                            if (!empty($cm_url) && $activity_name !== 'Actividad / Recurso eliminado') {
                                $activity_html = '<a href="'.$cm_url.'" target="_blank" style="color: #0056D2; text-decoration: none; transition: 0.2s;" onmouseover="this.style.color=\'#00C9A7\'" onmouseout="this.style.color=\'#0056D2\'"><i class="fa fa-external-link mr-1" style="font-size: 0.8rem;"></i>' . $activity_html . '</a>';
                            }
                        ?>
                        <tr>
                            <td class="font-weight-bold" style="font-size: 0.9rem; color: #444;"><?php echo $activity_html; ?></td>
                            <td><span style="background: #f0f0f0; color: #555; padding: 4px 8px; border-radius: 6px; font-size: 0.75rem; font-weight: bold; white-space: nowrap;"><?php echo htmlspecialchars($categoria_texto); ?></span></td>
                            <td><span class="badge-tipo bg-<?php echo $tipo_str; ?>"><?php echo $label_tipo; ?></span></td>
                            <td class="text-center font-weight-bold text-primary"><?php echo $log->amount; ?> XP</td>
    ...
    <?php 
        ...
        echo "<div class='text-center py-5 text-muted'>" . get_string('nopurchases', 'local_xpstore') . "</div>";
    }
    ?>
</div>
